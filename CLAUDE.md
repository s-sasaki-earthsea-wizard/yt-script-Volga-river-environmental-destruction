...
│   ├── yt_script_outline.md  # 動画アウトライン（完成）
...
アウトラインは `slides-jp/yt_script_outline.md` に記載。

### 全体構成

| セクション | 内容 | 目安時間 |
|-----------|------|---------|
| 1. フック | ソ連プロパガンダ風導入 → Beloye Morye の衝撃 | 2〜3分 |
| 2. 本編 | 川の重要性 → ソ連開発 → 汚染実態 → Beloye Morye → 修復の取り組み | 10〜12分 |
| 3. まとめ | 総括と個人的見解 | 1分以内 |
...
- **1-1**: ソ連プロパガンダ風スローガン（大げさな演技）
- **1-2**: Google Maps でヴォルガ川を探索
- **1-3**: 川の重要性を軽く紹介
- **1-4**: ソ連時代の開発を示唆
- **1-5**: Beloye Morye（白い海）の衝撃的な映像
- **1-6**: 「ここは化学兵器工場の産廃です」で本編へ

#### 2. 本編（2-1 〜 2-5）

- **2-1**: ヴォルガ川のスペック（ヨーロッパ最長、流域人口40%、チョウザメ被害）
- **2-2**: ソ連時代の開発（8つのダム、発電量、グラーグの影）
- **2-3**: 汚染の実態（排水40%集中、処理率10%、「もはや機械」引用）
- **2-4**: Beloye Morye 深掘り（28ha、150-350万トン、水銀・フェノール・ベンゾピレン）
- **2-5**: 環境修復の取り組み（ヘルシー・ヴォルガ 2,050億ルーブル、課題と情報の限界）
...
- Olson & Chernyanskii (2024) - Beloye Morye の詳細データ
- MIT Tech Review (2022.01.21) - 汚染統計、「もはや機械」引用
